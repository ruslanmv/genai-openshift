"""FastAPI application exposing Docling as a document-conversion service.

The API wraps the Docling CLI and provides endpoints that accept URLs or
uploaded files and return converted artefacts, suitable for wiring into
GenAI and RAG pipelines running on OpenShift.
"""
...
def docling_processing(
    ...
    """Run the Docling conversion pipeline for the given parameters and input files."""
...
def process_url(docling_params: DoclingParameters):
    """Download a document from the given URL and send it through the Docling conversion pipeline."""
...
def process_file(
    ...
    """Accept one or more uploaded files and run them through the Docling conversion pipeline."""
